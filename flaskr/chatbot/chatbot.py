...
class Chatbot(BaseQueryEngine):
    ...
    def __init__(self, model_name: str, DOCUMENT_ID, SCOPES):
        ...
        self.sub_llm = Ollama(model="llama3.2", request_timeout=120.0, json_mode=True)
        ...
        if self.document:
            self.index = prepare_documents(
                self.document, self.sub_llm, self.embed_model
            )
        else:
    ...
    def remove_think_blocks(self, response: str) -> Tuple[str, str]:
        ...
        return thinking_block, cleaned_text
